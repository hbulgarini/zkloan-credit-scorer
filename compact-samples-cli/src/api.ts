...
import { CompactSamples, type CompactSamplesPrivateState, witnesses } from '@midnight-ntwrk/counter-contract';
...
import {
  type CompactSamplesContract,
  type CompactSamplesPrivateStateId,
  type CompactSamplesProviders,
  type DeployedCompactSamplesContract,
} from './common-types';
...
export const getCompactSamplesLedgerState = async (
  providers: CompactSamplesProviders,
  ...
  const state = await providers.publicDataProvider
    ...
    .then((contractState) => (contractState != null ? CompactSamples.ledger(contractState.data).round : null));
  logger.info(`Ledger state: ${state}`);
  return state;
};

export const counterContractInstance: CompactSamplesContract = new CompactSamples.Contract(witnesses);

export const joinContract = async (
  providers: CompactSamplesProviders,
  contractAddress: string,
): Promise<DeployedCompactSamplesContract> => {
  const counterContract = await findDeployedContract(providers, {
    contractAddress,
    contract: counterContractInstance,
    privateStateId: 'counterPrivateState',
    initialPrivateState: { privateCompactSamples: 0 },
  });
...
export const deploy = async (
  providers: CompactSamplesProviders,
  privateState: CompactSamplesPrivateState,
): Promise<DeployedCompactSamplesContract> => {
...
export const increment = async (counterContract: DeployedCompactSamplesContract): Promise<FinalizedTxData> => {
...
export const displayCompactSamplesValue = async (
  providers: CompactSamplesProviders,
  counterContract: DeployedCompactSamplesContract,
): Promise<{ counterValue: bigint | null; contractAddress: string }> => {
  const contractAddress = counterContract.deployTxData.public.contractAddress;
  const counterValue = await getCompactSamplesLedgerState(providers, contractAddress);
...
export const configureProviders = async (wallet: Wallet & Resource, config: Config) => {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider<typeof CompactSamplesPrivateStateId>({
