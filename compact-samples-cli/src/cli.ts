...
import { type CompactSamplesProviders, type DeployedCompactSamplesContract } from './common-types';
...
const join = async (providers: CompactSamplesProviders, rli: Interface): Promise<DeployedCompactSamplesContract> => {
  const contractAddress = await rli.question('What is the contract address (in hex)? ');
  return await api.joinContract(providers, contractAddress);
};

const deployOrJoin = async (providers: CompactSamplesProviders, rli: Interface): Promise<DeployedCompactSamplesContract | null> => {
  while (true) {
    const choice = await rli.question(DEPLOY_OR_JOIN_QUESTION);
    switch (choice) {
      case '1':
        return await api.deploy(providers, { privateCompactSamples: 0 });
      case '2':
...
const mainLoop = async (providers: CompactSamplesProviders, rli: Interface): Promise<void> => {
  ...
  while (true) {
    ...
    switch (choice) {
      case '1':
        ...
        break;
      case '2':
        await api.displayCompactSamplesValue(providers, counterContract);
        break;
      case '3':
        logger.info('Exiting...');
        return;
