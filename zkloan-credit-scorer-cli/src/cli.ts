...
const deployOrJoin = async (providers: ZKLoanCreditScorerProviders, rli: Interface): Promise<DeployedZKLoanCreditScorerContract | null> => {
  while (true) {
    ...
    switch (choice) {
      case '1':
        return await api.deploy(providers, { privateZKLoanCreditScorer: 0 });
      case '2':
...
const mainLoop = async (providers: ZKLoanCreditScorerProviders, rli: Interface): Promise<void> => {
  ...
  while (true) {
    ...
    switch (choice) {
      case '1':
        ...
        break;
      case '2':
        await api.displayZKLoanCreditScorerValue(providers, counterContract);
        break;
      case '3':
        logger.info('Exiting...');
        return;
